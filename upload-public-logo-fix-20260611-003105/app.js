(function () {
  ...
  function renderRoundBlock(title, matches) {
    return '<div class="round-header"><span>' + title + '</span></div>' +
      matches.map(buildTile).join('');
  }
  ...
  function renderScrollableRoundBlock(title, matches) {
    return '<div class="match-row-section">' +
      ...
      '<div class="match-row-shell">' +
        ...
        '<div class="match-row-list" data-scroll-list tabindex="0" aria-label="' + title + '比赛列表">' + matches.map(buildTile).join('') + '</div>' +
