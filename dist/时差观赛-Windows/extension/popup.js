(function () {
  ...
  const SUPPORTED_URL_RE = /^(https:\/\/([^/]+\.)*(scgs\.tv|cctv\.com|cntv\.cn|yangshipin\.cn|xiaohongshu\.com)\/|http:\/\/(localhost|127\.0\.0\.1)(:\d+)?\/)/i;
  ...
  async function init() {
    ...
    toggle.addEventListener('change', async () => {
      ...
      updateUI(newState);
      await sendToggleMessage('nospoil_toggle', newState);
      window.close();
    });
    ...
    durationToggle.addEventListener('change', async () => {
      ...
      await sendToggleMessage('nospoil_duration_toggle', newState);
      window.close();
    });
  }
